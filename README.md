...
Инференс на ГПУ занимает меньше 1 минуты для маленькой модели и меньше 5 минут на большой. ГПУ используется автоматически при наличии.
Инференс на ЦПУ занимает 2 минут для маленькой модели и меньше 8 минут на большой. ЦПУ используется автоматически, если нет ГПУ.
...
- Дальше следует подгрузка графиков и таблиц с вкладом параметров. Из этих данных пользователь сможет понять вплоть до отдельных заказов, что повлияло на успех или срыв поставки. Здесь тоже приложены подробные инструкции как и что делать.
    ...
    ##### ВАЖНО: в работе с вкладом параметров, если используется ЦПУ(в особенности для больших моделей) 
    рекомендуется запустить вычисления на фон. Они могут занимать от 10 до 40 минут (для большой модели) и от 1 до 3 минут (для маленькой модели) в зависимости от используемого железа.
    на ГПУ такой проблемы нет, и маленькая и большая модель делает все вычисления моментально.
...
5. скачать папку models по ссылке https://drive.google.com/file/d/1JAkfGxBSga1lJc2nYn940LS0p07xTZmF/view?usp=sharing и загрузить ее в папку с кодом (если скачано с aiijc)
6. скачать папку stacking_model по ссылке https://drive.google.com/file/d/1I8BnqIHzylZRxKXmf8Kv5Q_PIOfpprkt/view?usp=sharing и загрузить ее в папку с кодом (если скачано с aiijc)
